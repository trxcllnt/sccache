...
fn sccache_server_cfg(
    ...
    config.toolchains_fallback.dir = Path::new(CONFIGS_CONTAINER_PATH).join(toolchains_path);
    config.toolchain_cache_size = TC_CACHE_SIZE as i64;
    config
}
...
impl DistSystem {
    ...
    pub fn add_scheduler(&mut self, message_broker: MessageBroker) {
        ...
        let scheduler_cfg = sccache_scheduler_cfg(&self.tmpdir, message_broker);
        ...
        let output = Command::new("docker")
            .args([
                ...
                "SCCACHE_NO_DAEMON=1",
                "-e",
                "SCCACHE_LOG=sccache=trace,tower_http=debug,axum::rejection=trace",
                "-e",
                "RUST_BACKTRACE=1",
                "--network",
                "host",
    ...
    pub fn add_server(&mut self, message_broker: MessageBroker) {
        ...
        let server_cfg = sccache_server_cfg(&self.tmpdir, message_broker);
    ...
    pub fn wait_server_ready(&mut self, handle: &ServerHandle) {
        let url = match handle {
            ServerHandle::Container { cid: _, url } => url.clone(), // ServerHandle::Process { pid: _, url } => {
                                                                    //     url.clone()
                                                                    // }
        };
