...
pub use self::internal::{Scheduler, SchedulerMetrics, SchedulerTasks};
...
mod internal {
    ...
    use crate::dist::{
        job_inputs_key, job_result_key, server_to_schedulers_queue, to_scheduler_queue,
    };
    ...
    impl SchedulerService for Scheduler {
        async fn get_status(&self) -> Result<SchedulerStatus> {
            ...
            Ok(SchedulerStatus {
                info: Some(servers.iter().fold(
                    dist::ServerStats::default(),
                    |mut info, server| {
