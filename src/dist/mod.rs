...
#[cfg(feature = "dist-server")]
pub use crate::dist::cache::ToolchainDiskCache;
...
pub enum RunJobResponse {
    ...
    JobComplete { result: BuildResult },
}
...
impl fmt::Display for ClientIncoming {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::RunJob(RunJobResponse::JobComplete { result }) => {
                write!(f, "Response::JobComplete({:?})", result.output)
            }
        }
    }
}
