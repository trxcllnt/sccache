...
impl ToolchainDiskCache {
    ...
    pub async fn acquire(&mut self, toolchain: &Toolchain) -> Result<PathBuf> {
        ...
        if !toolchain_is_loaded {
            ...
            loop {
                if self.cached_toolchains_size + inflated_size <= self.cached_toolchains_capacity {
                    trace!("ToolchainDiskCache: Unpacking toolchain {toolchain_id} to {path:?}");
                    async_tar::Archive::new(GzipDecoder::new(BufReader::new(toolchain_reader)).compat())
                        .unpack(&path)
                        .await
                        .context("Failed to unpack toolchain")?;
                    self.cached_toolchains_size += inflated_size;
                    trace!("ToolchainDiskCache: Toolchain {toolchain_id} unpacked, new cache size is {}", self.cached_toolchains_size);
                    break;
                }
    ...
    pub async fn release(&mut self, toolchain: &Toolchain) -> Result<()> {
        trace!(
            "ToolchainDiskCache: Releasing toolchain {}",
        ...
        for (toolchain, (inflated_size, _)) in toolchains_clone.iter().filter(|(_, (_, c))| *c <= 0) {
            let path = self
                .cached_toolchains_dir
                .join(make_lru_key_path(&toolchain.archive_id));
            toolchains.remove(toolchain);
            if path.exists() {
                trace!("ToolchainDiskCache: Removing toolchain dir {:?}", path);
