...
mod internal {
    ...
    use crate::{
        ...
        dist::{
            ...
            server_to_schedulers_queue, CompileCommand, RunJobError, RunJobResponse,
            SchedulerService, ServerDetails, ServerService, Toolchain,
        },
        errors::*,
    };
    ...
    impl Task for RunJob {
        ...
        async fn on_failure(&self, err: &TaskError) {
            ...
            {
                tracing::error!(
                    "[on_run_job_failure({job_id})]: Error reporting job failure: {err:#}"
                );
            }
        }
        ...
        async fn on_success(&self, res: &Self::Returns) {
            ...
            {
                tracing::error!(
                    "[on_run_job_success({job_id})]: Error reporting job success: {err:#}"
                );
            }
        }
    }
    ...
    impl Task for JobFinished {
        ...
        async fn run(&self, params: Self::Params) -> std::result::Result<Self::Returns, TaskError> {
            ...
            self.scheduler()
                ...
                .map_err(|e| {
                    tracing::error!(
                        "[job_finished({job_id})]: Failed with unexpected error: {e:#}"
                    );
