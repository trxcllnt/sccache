...
mod test {
    ...
    #[cfg(unix)]
    fn child_sees_cargo_makeflags(share: bool) -> Result<bool> {
        let client = Client::new_num(1);
        ...
        runtime.block_on(async {
            let output = crate::util::run_input_output(cmd, None).await?;
    ...
    #[test]
    #[cfg(unix)]
    fn jobserver_is_withheld_by_default() -> Result<()> {
        assert!(
    ...
    #[test]
    #[cfg(unix)]
    fn share_jobserver_hands_it_over() -> Result<()> {
        assert!(
